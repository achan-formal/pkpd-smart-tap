...
import json
import os
import app_complete as flask_server
import config
...
flow_ttime = 0.0
tap_on = False
current_mode = "default"  # default, wash, custom, lock
config.locked = False
...
def default_tap_operation():
    ...
    if config.locked:
        ...
        set_led("yellow", 0)  # Ensure yellow is off when locked
        return
    else:
    ...
    if config.api_tap_on == True:
        ...
        set_led("green", 1 if tap_on else 0)  # Update green LED based on water flow
        return            
        ...
        if not api_tapon_laststate == False:
            ...
            api_tapon_laststate = False
            if tap_on:  # Only beep if actually turning off
...
def main():
    ...
    flask_thread = threading.Thread(target=flask_server.run_flask)
